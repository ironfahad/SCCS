...
// const relocateTaskPriorityWise = (selectedTaskArray, e) => {

//   const priorityCellRange = resources.strategicSS().sheet.getRange(e.range.getRow(), 6); 
//   const priorityCellValue = priorityCellRange.getValue(); 
//   Logger.log(`Priority Cell value is ${priorityCellValue}`); 

//   const workingTaskArray = selectedTaskArray; 
//   Logger.log(workingTaskArray); // correct value so far


//   const tasksDataRange = resources.strategicSS().sheet.getRange(2,1,resources.strategicSS().sheet.getLastRow() - 1, strategicTasksSheet.getLastColumn()); // minus one is important otherwise a blank array will come
//   const totalTasksArray = tasksDataRange.getValues();  
//   Logger.log(totalTasksArray); // so far so good! 
...
//  for ( let i = 0; i < totalTasksArray.length; i++) {

//    let counterValue = 0; 

//    counterValue = counterValue + i 

//    if(counterValue <= 2 && priorityCellValue === "U") {

//      arraySlotU.push(totalTasksArray[i])

//    } else if (counterValue <= 5 && priorityCellValue === "A") {

//      arraySlotA.push(totalTasksArray[i])

//    } else if (counterValue <= 10 && priorityCellValue === "B") {

//      arraySlotB.push(totalTasksArray[i])

//    } else if (counterValue => 15 && priorityCellValue === "C") {

//      arraySlotC.push(totalTasksArray[i])

//    } else {
// b
//      Logger.log('Something wrong in the counter value for tasks array'); 
...
//   // for (let i = 11; i < totalTasksArray.length - 1; i++) {
...
//   // Logger.log(totalTasksArray); 

//   Logger.log(priorityCellValue); 
//   Logger.log(arraySlotU.length); 
//   Logger.log(arraySlotA.length); // problem is here! these are empty whereas these should not be empty! 
//   Logger.log(arraySlotB.length);// problem is here! these are empty whereas these should not be empty! 
//   Logger.log(arraySlotC.length);

//   Logger.log('Lalala 1 after for loop ')

//   // if (priorityCellValue.includes('U')) {
//   //     arraySlotU.unshift(workingTaskArray[0]); 
//   //   } else if(priorityCellValue.includes('A')) {
//   //     arraySlotA.unshift(workingTaskArray[0]); 
//   //   } else  if(priorityCellValue.includes('B')) {
//   //     arraySlotB.unshift(workingTaskArray[0]); 
//   //   } else if(priorityCellValue.includes('C')) {
//   //     arraySlotC.unshift(workingTaskArray[0]); 
//   //   } else {
//   //     arraySlotC.unshift(workingTaskArray[0]); 
//   //   }

//   Logger.log(arraySlotU.length); 
//   Logger.log(arraySlotA.length); 
//   Logger.log(arraySlotB.length);
//   Logger.log(arraySlotC.length);

//   Logger.log('Lalala 1 after includes ')

//     if(arraySlotU.length > 2) {
//       arraySlotA.unshift(arraySlotU[arraySlotU.length - 1]); 
//       arraySlotU.pop(); 
//     } else if (arraySlotA.length > 4) {
//       arraySlotB.unshift(arraySlotA[arraySlotA.length - 1]); 
//       arraySlotA.pop(); 
//     } else if (arraySlotB.length > 5) {
//       arraySlotC.unshift(arraySlotB[arraySlotB.length -1]); 
//       arraySlotB.pop(); 
//     };  
...
//   Logger.log('Lalala 1 after unshift ')

//     const finalPriorityArray = []; 

//     arraySlotU.forEach( row => {
//       if(row !== null) {
//       finalPriorityArray.push(row); 
//       }
//     }); 

//     arraySlotA.forEach( row => {
//       if(row !== null) {
//       finalPriorityArray.push(row); 
//       }
//     })

//     arraySlotB.forEach ( row => {
//       if(row !== null) {
//       finalPriorityArray.push(row); 
//       }
//     })

//     arraySlotC.forEach ( row => {
//       if(row !== null) {
//       finalPriorityArray.push(row); 
//       } 
//     }); 
...
//   Logger.log(arraySlotU); 
//   Logger.log(arraySlotA); 
//   Logger.log(arraySlotB);
//   Logger.log(arraySlotC);

//   Logger.log (`the length of finalpriorityarray is ${finalPriorityArray.length}`); 
//   Logger.log(finalPriorityArray); 

//   tasksDataRange.clearContent(); 
//   tasksDataRange.setValues(finalPriorityArray); 

//   const priorityNumbersRange = resources.strategicSS().sheet.getRange(2, 6, resources.strategicSS().sheet.getLastRow() - 1, 1); 
//   let priorityCellRangeArray = priorityNumbersRange.getValues();  
//   priorityNumbersRange.clearContent(); 

//   let newNumbersArray = []; 

//   Logger.log(priorityCellRangeArray.length); 

//   for (let i = 0; i < priorityCellRangeArray.length; i++) {
//     newNumbersArray.push(i); 
//   }; 

//   let twoDNumbersArray = []; 

//   while(newNumbersArray.length) {
//     twoDNumbersArray.push(newNumbersArray.splice(0,1)); 
//   }; 


//   Logger.log(priorityCellRangeArray)
//   Logger.log(twoDNumbersArray)


//   // Logger.log(newNumbersArray); 
//   priorityNumbersRange.clearContent(); 
//   // Logger.log(priorityCellRangeArray); 
//   // Logger.log(newNumbersArray); 
//   priorityNumbersRange.setValues(twoDNumbersArray); 
...
function detectPersonalTask(e) {
  ...
  if(strategicTasksSheet.getRange(1, e.range.getColumn()).getValue() === 'Delegate' && fun.getEventData(e).taskID > 0 ) {
   ...
   fun.unHighLightColor(resources.strategicId, resources.strategicSS().sheet.getSheetName(), fun.findRowNumber(resources.strategicId, resources.strategicSS().sheet.getSheetName(), taskArray[0][0])); 
    ...
    if(eventEmployeeName !== null || eventEmployeeName !== 'Me') {
